...
logger = logging.getLogger('learn_kmeans_on_hubert')

def get_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument("--feat-dir",type=str, help="store iter1 speciy layer feature of trained hubert")
...
def load_hubert_feature(feat_dir):
    feats = f"{feat_dir}/train-960_10_percent_hubert_raw_feature.npy" 
    feats = np.load(feats, mmap_mode="r") 
    logger.info(f"Loading hubert featuree from npy file succcessfully, feats shape: {feats.shape}")
...
def main(args):
    ...
    feats = load_hubert_feature(feat_dir=args.feat_dir)
