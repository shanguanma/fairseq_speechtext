...
if [ ${stage} -le 16 ] && [ ${stop_stage} -ge 16 ];then
  ...
  # Unpaired text input 
  TEXT_DATA=$des_dir/librispeech_lm_norm_phn_seq/phonesss ## it offers phone dictionary, its type is as follow:
  ...
  ## IY 138376
  ## Z 128619
  cp -r  dataset/format/librispeech/librispeech_lm_norm_phn_seq/phoness/{train.bin,train.idx} $TEXT_DATA  ## unpaire phonemem level text only as trainset.
  ...
  world_size=1
  update_freq=1
  CUDA_VISIBLE_DEVICES=4  python $fairseq_dir/fairseq_cli/hydra_train.py  --multirun \
     --config-dir $config_dir/config/gan \
     --config-name w2vu2_local \
     task.data=${TASK_DATA} \
...
if [ ${stage} -le 20 ]&& [ ${stop_stage} -ge 20 ];then
   ...
   fairseq_dir=/workspace2/maduo/fairseq_speechtext
   root_dir=/workspace2/maduo/dataset/format/librispeech
   tsv_dir=$root_dir/librispeech ### it contains *.wrd and *.phn, because it doesn't matter whether the wav contains silence or not 
   feat_dir=$root_dir/wav2vec_large_feat_dir_no_silence
   ## dict.phn.txt contain two columns ,first column is monophone(total is 41 mono phones, last monophone is <SIL>), second column is index(0-base),
   cp -r $root_dir/librispeech_lm_norm_phn_seq/phoness/dict.phn.txt $feat_dir
   config_dir=$fairseq_dir/examples/wav2vec/
...
if [ ${stage} -le 21 ]&& [ ${stop_stage} -ge 21 ];then
   echo "decode  dev-clean dev-other test-clean test-other using wav2vec-u2.0 model and compute PER "
   echo "with phn2word  kenlm model"
   fairseq_dir=/workspace2/maduo/fairseq_speechtext
   tsv_dir=/workspace2/maduo/dataset/format/librispeech
   feat_dir=$tsv_dir/wav2vec_large_feat_dir_no_silence
   ## dict.phn.txt contain two columns ,first column is monophone(total is 41 mono phones, last monophone is <SIL>), second column is index(0-base),
   ...
   wav2vec_u2_dir=$exp_dir
   dest_dir=$dir/wav2vec-u2/hyps
   path_to_lmmodel=dataset/format/librispeech/librispeech_lm_norm_phn_seq/kenlm.wrd.o40003.bin  ##word lm model
   path_to_lexicon=dataset/format/librispeech/librispeech_lm_norm_phn_seq/lexicon_filtered.lst ##word2phn  lexicon
   mkdir -p $dest_dir
   testsets="dev-clean dev-other test-clean test-other"
   #testsets="test-other"
   export PYTHONPATH=$fairseq_dir:$PYTHONPATH
   for name in $testsets;do
       ...
       python $fairseq_dir/examples/wav2vec/unsupervised/w2vu_generate.py\
               ...
               word_score=1.0\
               sil_weight=0.0\
               lm_model=$path_to_lmmodel\
               lexicon=$path_to_lexicon
...
if [ ${stage} -le 25 ] && [ ${stop_stage} -ge 25 ];then
  ...
  # Unpaired text input
  TEXT_DATA=$des_dir/librispeech_lm_norm_phn_seq/spilt_three ## it  offers unpair trainset(train.bin, train.idx), devset (dev-clean.bin.dev-clean.idx, dev-other.bin, dev-other.idx)
  ...
  dir=/workspace2/maduo/exp/
  model_name=w2v_unsup_gan_xp_1gpu_1update_include_unpair_textdevset
  exp_dir=$dir/wav2vec-u2_gan_from_scratch/${model_name}
  mkdir -p $exp_dir
  export PYTHONPATH=$fairseq_dir:$PYTHONPATH
  world_size=1
  update_freq=1
  CUDA_VISIBLE_DEVICES=6  python $fairseq_dir/fairseq_cli/hydra_train.py  --multirun \
      --config-dir $config_dir/config/gan \
     --config-name w2vu2_local \
     task.data=${TASK_DATA} \
     task.text_data=${TEXT_DATA} \
...
if [ ${stage} -le 30 ] && [ ${stop_stage} -ge 30 ];then
  ...
  # Unpaired text input
  TEXT_DATA=$des_dir/librispeech_lm_norm_phn_seq/spilt_three_half ## it  offers unpair trainset(train.bin, train.idx), devset (dev-clean.bin.dev-clean.idx, dev-other.bin, dev-other.idx)
  # it also offers phone dictionary, its type is as follow:
  ##head  dataset/format/librispeech/librispeech_lm_norm_phn_seq/phonesss/dict.phn.txt
  ## AH 336212
  ## N 238016
  ...
  dir=/workspace2/maduo/exp/
  model_name=w2v_unsup_gan_xp_1gpu_1update_with_include_unpair_textdevset_half
  exp_dir=$dir/wav2vec-u2_gan_from_scratch/${model_name}
  mkdir -p $exp_dir
  export PYTHONPATH=$fairseq_dir:$PYTHONPATH
  world_size=1
  update_freq=1
  CUDA_VISIBLE_DEVICES=5  python $fairseq_dir/fairseq_cli/hydra_train.py  --multirun \
      --config-dir $config_dir/config/gan \
     --config-name w2vu2_local \
     task.data=${TASK_DATA} \
     task.text_data=${TEXT_DATA} \
...
if [ ${stage} -le 35 ] && [ ${stop_stage} -ge 35 ];then
  ...
  dir=/workspace2/maduo/exp/
  model_name=w2v_unsup_gan_xp_4gpu_8update_with_unpair_text_half
  exp_dir=$dir/wav2vec-u2_gan_from_scratch/${model_name}
  mkdir -p $exp_dir
  export PYTHONPATH=$fairseq_dir:$PYTHONPATH
  world_size=1
...
if [ ${stage} -le 36 ]&& [ ${stop_stage} -ge 36 ];then
   ...
   for name in $testsets;do
       ...
       python $fairseq_dir/examples/wav2vec/unsupervised/w2vu_generate.py\
               ...
               fairseq.common.user_dir=${fairseq_dir}/examples/wav2vec/unsupervised \
               fairseq.task.data=$feat_dir \
               fairseq.common_eval.path=$wav2vec_u2_dir/checkpoint_best.pt \
               fairseq.common_eval.quiet=false \
               fairseq.dataset.gen_subset=$name\
               results_path=$dest_dir\
               decode_stride=2\
   ...
   ## PER% log: grep -rn "WER" logs/wav2vec-u2_from_scratch_stage36_37.log 
   ## beam=1500, beam_threshold=100.0
   ## dev-clean dev-other  test-clean test-other
   ##  14.44    18.63        15.04      18.59 
...
if [ ${stage} -le 37 ]&& [ ${stop_stage} -ge 37 ];then
   ...
   ## WER% , log: grep -rn "WER:" logs/wav2vec-u2_from_scratch_stage37.log 
   ## dev-clean dev-other test-clean test-other
   ## 17.39       21.42    18.15       22.17

fi
if [ ${stage} -le 38 ]&& [ ${stop_stage} -ge 38 ];then
   echo "decode without vad dev-clean dev-other test-clean test-other using wav2vec-u2.0 model and compute PER "
   fairseq_dir=/workspace2/maduo/fairseq_speechtext
   root_dir=/workspace2/maduo/dataset/format/librispeech
...
if [ ${stage} -le 39 ]&& [ ${stop_stage} -ge 39 ];then
   echo "decode without vad dev-clean dev-other test-clean test-other using wav2vec-u2.0 model and compute WER with hlg graph(phn2word)"
   fairseq_dir=/workspace2/maduo/fairseq_speechtext
   root_dir=/workspace2/maduo/dataset/format/librispeech
   tsv_dir=$root_dir ### it contains *.wrd and *.phn, because it doesn't matter whether the wav contains silence or not
   feat_dir=$root_dir/wav2vec_large_feat_dir
   ...
   wav2vec_u2_dir=$dir/wav2vec-u2_gan_from_scratch/w2v_unsup_gan_xp_4gpu_8update_with_unpair_text_half/
   dest_dir=$wav2vec_u2_dir/hyps_debug_for_without_vad_and_kaldi_decoder
   mkdir -p $dest_dir
   testsets="dev-clean dev-other test-clean test-other"
   #testsets="dev-clean" ## for debug
   export PYTHONPATH=$fairseq_dir:$PYTHONPATH
...
if [ ${stage} -le 40 ] && [ ${stop_stage} -ge 40 ];then
  ...
  # Unpaired text input
  TEXT_DATA=$des_dir/librispeech_lm_norm_phn_seq/unpair_text_lasthalf ## it  offers unpair trainset(train.bin, train.idx), devset (dev-clean.bin.dev-clean.idx, dev-other.bin, dev-other.idx)
  # it also offers phone dictionary, its type is as follow:
  ##head  dataset/format/librispeech/librispeech_lm_norm_phn_seq/phonesss/dict.phn.txt
  ## AH 336212
  ## N 238016
  ...
  dir=/workspace2/maduo/exp/
  model_name=w2v_unsup_gan_xp_4gpu_8update_with_unpair_text_lasthalf
  exp_dir=$dir/wav2vec-u2_gan_from_scratch/${model_name}
  mkdir -p $exp_dir
  export PYTHONPATH=$fairseq_dir:$PYTHONPATH
  world_size=1
  update_freq=1
  CUDA_VISIBLE_DEVICES=3  python $fairseq_dir/fairseq_cli/hydra_train.py  --multirun \
       --config-dir $config_dir/config/gan \
     --config-name w2vu2_local \
     task.data=${TASK_DATA} \
     task.text_data=${TEXT_DATA} \
