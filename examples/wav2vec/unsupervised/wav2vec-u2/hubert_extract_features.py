...
def get_parser():
    ...
    parser.add_argument('--wavfile-suffix', type=str, default=".tsv", help='wav file format. i.e.: .scp or .tsv')
    # fmt: on
...
def get_iterator(args):
    with open(osp.join(args.data, args.split) + args.wavfile-suffix, "r") as fp:
        files=[]
        if args.wavfile-suffix == ".scp":
            lines = fp.read().split("\n")
            files = [line.split()[1] for line in lines if len(line) > 0]

        elif args.wavfile-suffix == ".tsv":
            lines = fp.read().split("\n")
