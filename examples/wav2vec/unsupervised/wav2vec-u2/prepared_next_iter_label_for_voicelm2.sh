...
if [ ${stage} -le 2 ]&& [ ${stop_stage} -ge 2 ];then
   echo "dump voicelm2 feature of audio into local for training k-means model"
    ...
    #for name in dev-clean train-960;do
       ...
       for c in 500;do
          for layer in $layers;do
         label_dir=dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/voicelm2_${layer}layer_${c}_clusters_label_dir
         km_model_path=dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/train-960_10_percent_voicelm2_${layer}layer_km_${c}_clusters.mdl
         mkdir -p $label_dir
   ...
   echo "dump voicelm2 feature of audio into local for training k-means model"
    ...
    #for name in dev-clean train-960;do
       ...
       for  c in $clusters; do
         for layer in $layers ;do
         label_dir=dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/voicelm2_${layer}layer_${c}_clusters_label_dir_for_phmi
         km_model_path=dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/train-960_10_percent_voicelm2_${layer}layer_km_${c}_clusters.mdl
         mkdir -p $label_dir
        CUDA_VISIBLE_DEVICES=6 \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ];then
   ...
   tsv_dir=/workspace2/maduo/dataset/format/librispeech
   #lab_dir=$tsv_dir/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/voicelm2_${layer}layer_${c}_clusters_label_dir
   n_clusters=500
   layers=12
   for layer in $layers;do
    for n_cluster in $n_clusters;do
     lab_dir=$tsv_dir/feat_dir/voicelm2_feat_of_0.1_960h_librispeech/voicelm2_${layer}layer_${n_cluster}_clusters_label_dir
