...
class Voicelm2FeatureReader(object):
    def __init__(
        ...
        (
            ...
            self.task.cfg.min_sample_size,
            manifest_text_path,
            self.task.cfg.max_phone_size,
            self.task.cfg.min_phone_size,
...
def get_parser():
    ...
    parser.add_argument(
        "--feat_dir",
        type=str,
        default="dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Controls the number of features required by kmeans model. If ratio=10, it means that only one-eleventh of the features are used for kmeans clustering.",
    )
    return parser
...
if __name__ == "__main__":
    ...
    ## dump feature for training kmeans model
    #feat_dir="dataset/format/librispeech/feat_dir/voicelm2_feat_of_0.1_960h_librispeech"
    feat_path = f"{args.feat_dir}/train-960_10_percent_voicelm2_{args.layer}layer_raw_feature.npy"
    os.makedirs(args.feat_dir,exist_ok=True)
    if os.path.exists(feat_path):
        os.remove(feat_path)
    feat_f  = NpyAppendArray(feat_path) 
    ## use 1/10 audio feature to train k-means model.
    utts = reader.sizes()//args.ratio ## int
    #utts = reader.sizes()
    
    
    for i in tqdm(range(utts),desc="feature"):
        feat = reader.get_feats(i)
        #print(f"feat: {feat}??????")
        #logging.info(f"feat: {feat}!!!!!")
        #logging.info(f"feat shape: {feat.shape}, feat: {feat}")
        feat_f.append(feat.cpu().numpy())
    logging.info(f"Dump voicelm2 feature successfully, it is at {feat_path}") 
