...
import torch
import tqdm
from common import MfccFeatureReader,MfccFeatureReaderWavscp, get_path_iterator, get_path_iterator_for_wavscp, train_km_model, learn_kmeans
...
def get_mfcc_feature(wavscp, sample_rate, nj, portion):
    reader = MfccFeatureReaderWavscp(sample_rate)
    generator, num = get_path_iterator_for_wavscp(wavscp, portion)
    iterator = generator()
    if nj>1:
...
def load_feature(
    ...
    feat = get_mfcc_feature(wavscp, sample_rate, nj, portion)
    return feat
