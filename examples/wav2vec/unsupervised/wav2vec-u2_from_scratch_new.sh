...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ];then
  ...
  python $fairseq_dir/fairseq_cli/hydra_train.py\
     ...
     hydra.run.dir=$exp_dir\
     hydra.job.name=$exp_dir/w2v_unsup_gan_xp
# util run 110000 steps, I will stop it
# grep -rn "dev-other_uer" exp/wav2vec-u2_gan_from_scratch/w2v_unsup_gan_xp_1gpu_1update_with_unpair_text_all/w2v_unsup_gan_xp.log | awk -F ":" '{print $12 $13}' | awk -F "," '{print $2}' | sort
# best dev-other-uer: 81.6807
fi

#(TODO) run it using md config in English case.
