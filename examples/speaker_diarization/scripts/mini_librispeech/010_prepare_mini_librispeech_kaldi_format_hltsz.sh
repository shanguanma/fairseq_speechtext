...
if [ $stage -le 0 ]&& [ ${stop_stage} -ge 0 ]; then
    ...
    mini_librispeech_url=https://us.openslr.org/resources/31
    mkdir -p data/local
    source_md/download_and_untar_mini_librispeech.sh data/local $mini_librispeech_url  dev-clean-2

    source_md/download_and_untar_mini_librispeech.sh data/local $mini_librispeech_url train-clean-5
...
if [ $stage -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    random_mixture_cmd=source_md/random_mixture_nooverlap.py
    make_mixture_cmd=source_md/make_mixture_nooverlap.py
    if [ "$simu_opts_overlap" == "yes" ]; then
        random_mixture_cmd=source_md/random_mixture.py
        make_mixture_cmd=source_md/make_mixture.py
    fi
