...
def get_args():
    ...
    parser.add_argument(
        "--path_librispeech",
        help="the path to root of the LibriSpeech dataset, eg. ....../LibriSpeech/",
    )
    # parser.add_argument("--tsv_folder", help="the path to the tsv file of the split, e.g. ../Libri2Mix/wav16k/max/tsv_files/")
    parser.add_argument(
        "--path_librimix", help="the path to the librimix dataset e.g. ../wav16k/max/"
    )
    parser.add_argument(
        "--output_folder",
        help="the relative path to write the aux file inside the Librimix folder, e.g. tsv_files",
    )
    parser.add_argument(
        "--type",
        choices=["train", "test"],
        help="split of the dataset, it can be train or test",
    )
    parser.add_argument(
        "--random_seed",
        type=int,
        help="Random seed for reproducibility, default value is 42",
        default=42,
    )
    ...
    args.path_librispeech = os.path.join(
        args.path_librispeech,
...
def main(args):
    ...
    with open(output_path, "w") as output_file:
        for uttid in tqdm(uttid_list):
            ...
            for librispeech_id in librispeech_ids:
                ...
                currid_folder = os.path.join(
                    args.path_librispeech, curr_list[0], curr_list[1]
                )
                assert os.path.exists(
                    currid_folder
                ), f"{librispeech_id} led to folder {currid_folder} but this folder does not exist"

                all_uttids = [
                    elem.split(".")[0]
                    for elem in os.listdir(currid_folder)
                    if elem.endswith(".flac")
                ]
                assert (
                    librispeech_id in all_uttids
                ), f"{librispeech_id} led to folder {currid_folder} but this folder does not have this utterance"

                all_uttids.remove(librispeech_id)

                aux_id = random.choice(all_uttids)
                assert (
                    aux_id != librispeech_id
                ), f"{aux_id=} cannot be equal to the {librispeech_id=}"

                full_aux_path = os.path.abspath(
                    os.path.join(currid_folder, f"{aux_id}.flac")
                )

                full_uttid_path = os.path.abspath(
                    os.path.join(args.path_librimix, f"{uttid}.wav")
                )
        ...
        print(
            f"The dataset consists of {len(processed)} mixtures with folowing number of speakers: {numspk_list}"
        )
    ...
    if args.type == "train":
        ...
        with open(output_path, "w") as output_file:
            ...
            print(
                f"The DEV dataset consists of {len(processed)} mixtures with folowing number of speakers: {numspk_list}"
            )
