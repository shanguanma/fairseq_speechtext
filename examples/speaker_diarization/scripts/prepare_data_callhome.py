	...
	for uttid in tqdm(segments):
		...
		for key in intervals:
			...
			for interval in intervals[key]:
				s, e = interval
				for i in range(int(s * args.rate), min(int(e * args.rate) + 1, len(labels))):
					labels[i] = 1
			...
			res = {'filename':id_full, 'speaker_key':key, 'speaker_id': speaker_id, 'labels': labels, 'real_data': "true", 'aux': speaker_id, 'num_spk': 6}
			json.dump(res, outs)
			outs.write('\n')
