...
def main():
    ...
    os.makedirs(args.target_audio_path, exist_ok=True)
    skip_file = open(os.path.join(args.target_audio_path, "skip_wavs.txt"), "a")
    for room_id in tqdm.tqdm(room_set):
        output_dir = os.path.join(args.target_audio_path, room_id)
        ...
        for key in new_intervals:
            ...
            if len(new_audio) <= 10:
                print(f"{room_id} {len(new_audio)}", file=skip_file)
                break
            if args.sample_rate != 16000:
                try:
                    new_audio_re = signal.resample(
                        new_audio, int((16000 * len(new_audio)) / args.sample_rate)
                    )  # upsample to 16000
                    print(f"{room_id} {len(new_audio_re)}")
            ...
            if wav_length > int(args.length_embedding * 16000):
                for start in range(
                    ...
                    target_speech = torch.FloatTensor(
                        numpy.array(new_audio_re[start:stop])
                    )
