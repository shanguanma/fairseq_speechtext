...
audio_dir = (
    "/workspace2/junyi/datasets/callhome_sim/--no-use-rirs--use-noises/validation/wavs"
)
...
def convert(audio_path):
    ...
    audio_re = scipy.signal.resample(
        audio, int((16000 * len(audio)) / 8000)
    )  # upsample to 16000
...
Parallel(n_jobs=80)(
    delayed(convert)(audio_path) for audio_path in glob(audio_dir + "/*.wav")
)
