...
from examples.speaker_diarization.ts_vad.models.modules.speakerEncoder import ECAPA_TDNN, PreEmphasis
from examples.speaker_diarization.ts_vad.models.modules.cam_pplus import CAMPPlus
from examples.speaker_diarization.ts_vad.models.modules.postional_encoding import PositionalEncoding
from examples.speaker_diarization.ts_vad.models.modules.WavLM import WavLM, WavLMConfig
from examples.speaker_diarization.ts_vad.models.modules.self_att import CoAttention_Simple
from examples.speaker_diarization.ts_vad.models.modules.joint_speaker_det import JointSpeakerDet
...
class TSVADConfig(FairseqDataclass):
    ...
    use_lstm_single: bool = field(
        default=False,
        metadata={"help": "use lstm for single"}
    )
    use_lstm_multi: bool = field(
        default=False,
        metadata={"help": "use lstm for multi"}
    )
    use_spk_embed: bool = field(
        default=True,
        metadata={"help": "whether to use speaker embedding"}
    )
    add_ind_proj: bool = field(
        default=False,
        metadata={"help": "whether to add projection for each speaker"}
    )
...
class TSVADModel(BaseFairseqModel):
    def __init__(
        ...
        if task_cfg.label_rate != 25:
            assert task_cfg.speech_encoder_type == "ecapa" or task_cfg.speech_encoder_type == "cam", 'Only support ecapa and cam++ for label rate not 25'
        ...
        if self.speech_encoder_type == "wavlm":
            ...
            self.speech_down = nn.Sequential(
                nn.Conv1d(768, cfg.speaker_embed_dim, 5, stride=int(2 // sample_times), padding=2),
                BatchNorm1D(num_features=cfg.speaker_embed_dim),
                nn.ReLU(),
            )
        elif self.speech_encoder_type == "ecapa":
            self.speech_encoder = ECAPA_TDNN(C=1024, dropout=cfg.dropout, speech_encoder=True)
            self.speech_encoder.train()
            if cfg.speech_encoder_path is not None:
                self.load_speaker_encoder(cfg.speech_encoder_path, module_name='speech_encoder')
            else:
            ...
            self.torchfbank = torch.nn.Sequential(
                PreEmphasis(),            
                torchaudio.transforms.MelSpectrogram(sample_rate=task_cfg.sample_rate, n_fft=512, win_length=400, hop_length=160, \
                                                    f_min = 20, f_max = 7600, window_fn=torch.hamming_window, n_mels=80),
            )
            self.speech_up = nn.Sequential(
                nn.Conv1d(80, cfg.speaker_embed_dim, 5, stride=int(4 // sample_times), padding=2),
                BatchNorm1D(num_features=cfg.speaker_embed_dim),
                nn.ReLU(),
            )
        ...
        self.pos_encoder = PositionalEncoding(cfg.transformer_embed_dim, dropout=cfg.dropout, max_len=(task_cfg.rs_len * self.label_rate))
        ...
        if self.use_jsd_block:
            ...
            self.backend_down = nn.Sequential(
                nn.Conv1d(cfg.transformer_embed_dim * task_cfg.max_num_speaker, cfg.transformer_embed_dim, 5, stride=1, padding=2),
            ...
            if cfg.add_ind_proj:
                self.pre_fc = nn.ModuleList([nn.Linear(cfg.transformer_embed_dim, 192) for _ in range(task_cfg.max_num_speaker)])
        ...
        if task_cfg.support_mc:
            self.co_attn = CoAttention_Simple(out_channels=192, embed_dim=256, num_heads=256)
    ...
    def rs_forward(self, x, max_len, fix_encoder=False): # B, 25 * T
        ...
        assert x.size(-1) - max_len <= 2 and x.size(-1) - max_len >= -1, f"label and input diff: {x.size(-1) - max_len}"
    ...
    def cat_multi_forward(self, cat_embeds):
        ...
        # Downsampling
        if self.use_jsd_block:
            cat_embeds = cat_embeds.reshape(B, self.max_num_speaker, T, -1) # B, 4 * 512, T
    ...
    def calculate_loss(self, outs, labels, labels_len):
        ...
        for i in range(labels_len.size(0)):
            total_loss += self.loss(outs[i, :, :labels_len[i]], labels[i, :, :labels_len[i]])
    ...
    def forward(
        ...
        rs_embeds = self.rs_forward(ref_speech, labels.size(-1), fix_encoder=(self.num_updates < self.freeze_speech_encoder_updates and fix_encoder))
        ...
        if self.add_ind_proj:
            outs_pre = torch.stack([self.pre_fc[i](outs_pre) for i in range(self.max_num_speaker)], dim=1)
            outs = self.fc(outs_pre.view(-1, labels.size(-1), 192)).view(ref_speech.size(0), self.max_num_speaker, labels.size(-1)).transpose(1, 2)
        else:
        ...
        mi, fa, cf, acc, der = self.calc_diarization_result(outs_prob.transpose((0, 2, 1)), labels.transpose(1, 2), labels_len)
    ...
    def load_speaker_encoder(self, model_path, module_name="speaker_encoder"):
        ...
        for name, param in loadedState.items():
            ...
            if module_name == "speaker_encoder" and hasattr(self.speaker_encoder, "bn1") and isinstance(self.speaker_encoder.bn1, BatchNorm1D) and '.'.join(name.split('.')[:-1]) + '.running_mean' in loadedState:
                name = '.'.join(name.split('.')[:-1]) + '.bn.' + name.split('.')[-1]

            if module_name == "speech_encoder" and hasattr(self.speech_encoder, "bn1") and isinstance(self.speech_encoder.bn1, BatchNorm1D) and '.'.join(name.split('.')[:-1]) + '.running_mean' in loadedState:
                name = '.'.join(name.split('.')[:-1]) + '.bn.' + name.split('.')[-1]
