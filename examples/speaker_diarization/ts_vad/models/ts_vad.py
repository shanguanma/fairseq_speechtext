...
class TSVADConfig(FairseqDataclass):
    ...
    wavlm_pretrain_model: str = field(
        default="/mntcephfs/lab_data/maduo/model_hub/speaker_pretrain_model/wavlm/WavLM-Large.pt",
        metadata={
            "help": "SSL model checkpoint, it is used to finetune speaker verification model, It is only used to instantiate the pre-trained model, and the final parameters come from the fine-tune model"
        },
    )
    update_extract: bool = field(default=False, metadata={"help":"Whether to update the SSL network parameters of the speaker model when training ts_vad"})
...
class TSVADModel(BaseFairseqModel):
    def __init__(
        ...
        if self.speech_encoder_type == "wavlm":
        ...
        elif self.speech_encoder_type == "wavlm_large_ft":
            self.speech_encoder = ECAPA_TDNN_SMALL(
                feat_dim=1024,
                emb_dim=256,
                feat_type="wavlm_large",
                update_extract=cfg.update_extract,
                pretrain_ckpt=cfg.wavlm_pretrain_model,
            )
            self.speech_encoder.train()
            ...
            # because wavlm output frames rate is 50frames/1s, so I need to downsample to 25frames/1s.
            self.speech_down = nn.Sequential(
                nn.Conv1d(
                    1536, # because it is from output feature dimension before pool of speech_encoder
                    cfg.speaker_embed_dim, # expect it is 256
                    5,
                    stride=int(2 // sample_times),
                    padding=2,
                ),
    ...
    def rs_forward(self, x, max_len, fix_encoder=False):  # B, 25 * T
        ...
        if self.speech_encoder_type == "wavlm":
            ...
            self.speech_encoder_type == "resnet34_wespeaker"
        ):  # its input is fbank,it is extract from data/ts_vad_dataset.py
            with torch.no_grad() if fix_encoder else contextlib.ExitStack():
                x = self.speech_encoder(x, get_time_out=True)
            x = self.speech_up(x)
