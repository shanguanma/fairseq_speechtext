...
class ChannelWiseLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        """
        if x.dim() != 3:
            raise RuntimeError("ChannelWiseLayerNorm accept 3D tensor as input")
        # N x C x T => N x T x C
...
class Conv1DBlock_v4(nn.Module):
    ...
    def __init__(
        ...
        self.dconv_pad = dconv_pad
        # Film
        self.film_gamma = nn.Conv1d(film_channel, in_channels, 5, stride=1, padding=2)
    ...
    def forward(self, x, diar_probs):
        ...
        film_be = self.film_beta(diar_probs)
        y = film_ga * y + film_be
        x = x + y
        return x
