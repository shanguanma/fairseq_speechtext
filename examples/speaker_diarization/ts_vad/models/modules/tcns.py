...
class TCN(nn.Module):
    def __init__(self, spk_embed_dim, enc_out_size, conv_out_channels, conv_kernel_size, conv_block_norm, total_conv_block_num, conv_block_num, num_spk=None, spk_embed_input=False):
        ...
        if num_spk is not None:
            self.mask_module = nn.ModuleList([Conv1D(enc_out_size, enc_out_size, 1) for _ in range(num_spk)])
