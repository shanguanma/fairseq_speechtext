...
class DualPathRNN(nn.Module):
    def __init__(self, N, dprnn_output_channel, dprnn_hidden_state, dprnn_chunk_size, dprnn_layers, spk_embed_dim):
        # N, B, H, K, R
        ...
        self.prelu = nn.PReLU()
        self.av_conv = nn.Conv1d(dprnn_output_channel + spk_embed_dim, dprnn_output_channel, 1, bias=False)
    ...
    def forward(self, x, spk_emb):
        ...
        spk_emb = torch.repeat_interleave(spk_emb.unsqueeze(2), repeats=x.size(2), dim=2)
...
class DualRNNBlock(nn.Module):
    def __init__(self, out_channels, hidden_channels, rnn_type='LSTM', dropout=0, bidirectional=False):
