...
class FBank(object):
    ...
    def __call__(self, wav, dither=1.0):
        ...
        feat = kaldi.fbank(wav, num_mel_bins=self.n_mels,
            sample_frequency=sr, dither=dither, window_type='hamming', use_energy=False)
        # feat: [T, N]
        if self.mean_nor:
...
def _collate_data(
    frames, is_audio_input: bool = False, is_label_input: bool = False, is_embed_input: bool = False
) -> torch.Tensor:
    if is_audio_input:
    ...
    if is_embed_input:
        if len(frames[0].size()) == 2:
            ...
            max_len = max(frame.size(1) for frame in frames)
            out = frames[0].new_zeros((len(frames), frames[0].size(0), max_len, frames[0].size(2)))
            for i, v in enumerate(frames):
                out[i, :, : v.size(1), :] = v
...
class TSVADDataset(FairseqDataset):
    def __init__(
        ...
        for line in tqdm(lines):
            ...
            if self.dataset_name == 'alimeeting' or self.dataset_name == 'icmc':
            ...
            elif self.dataset_name == 'ami' or self.dataset_name == 'callhome_sim' or self.dataset_name == 'libri_css_sim':
                self.spk2data[speaker_id_full].append(filename + '/' + speaker_id_full)
                self.data2spk[filename + '/' + speaker_id_full] = speaker_id_full
            elif re.match("^SparseLibri(2|3|23)Mix$|^Libri(2|3|23)Mix$", self.dataset_name):
                ...
                raise Exception(f"The given dataset {self.dataset_name} is not supported.")
            ...
            if filename in filename_set:
                ...
                elif self.dataset_name == 'callhome_sim' or self.dataset_name == 'icmc' or self.dataset_name == 'libri_css_sim':
                ...
                if inference: # only for joint model
                    self.data_list.append([filename, num_speaker, 0, length - 1, real_data])
                    ...
                    for start in range(0, length, dis):
                        ...
                        if end - start > label_rate * short_ratio:
                            ...
                            self.sizes.append((end - start) * self.sample_rate / label_rate)
        ...
        if musan_path is not None:
            ...
            augment_files   = glob.glob(os.path.join(musan_path,'*/*/*.wav')) # musan/*/*/*.wav 
        ...
        if rir_path is not None:
            ...
            self.rir_files = glob.glob(os.path.join(rir_path,'*/*.wav')) # RIRS_NOISES/*/*.wav 
    ...
    def __getitem__(self, index):
        ...
        ref_speech, labels, new_speaker_ids, _ = self.load_rs(file, speaker_ids, start, stop)
        if self.embed_input:
    ...
    def load_rs(self, file, speaker_ids, start, stop):
        ...
        if re.match("^SparseLibri(2|3|23)Mix$|^Libri(2|3|23)Mix$", self.dataset_name) or self.dataset_name == 'callhome_sim' or self.dataset_name == 'libri_css_sim':
        ...
        if self.dataset_name == 'icmc' and self.support_mc:
            ref_speech_list = []
            for i in range(4):
                ref_speech_list.append(self.read_audio_with_resample(os.path.join(self.audio_path, file + f'/DX0{i + 1}C01.wav'), start=audio_start, length=(audio_stop - audio_start))[0])
            
            ref_speech = np.stack(ref_speech_list)
            rc = -1
        else:
            ref_speech, rc = self.read_audio_with_resample(audio_path, start=audio_start, length=(audio_stop - audio_start), support_mc=self.support_mc)
            if len(ref_speech.shape) == 1:
                ref_speech = np.expand_dims(np.array(ref_speech), axis = 0)

        frame_len = audio_stop - audio_start

        assert frame_len - ref_speech.shape[1] <= 100, f"frame_len {frame_len} ref_speech.shape[1] {ref_speech.shape[1]}"
        ...
        for speaker_id in speaker_ids:
            ...
            if self.random_mask_speaker_prob != 0:
                mask_prob = self.random_mask_speaker_prob * min(self.update_num / self.random_mask_speaker_step, 1.0)
            if sum(labels[-1]) == 0 and self.is_train:
                new_speaker_ids.append(-1)
            elif sum(new_speaker_ids) != -1 * len(new_speaker_ids) and np.random.choice(2, p=[1 - mask_prob, mask_prob]) and self.is_train:
                new_speaker_ids.append(-1)
    ...
    def load_ts_embed(self, file, speaker_ids):
        ...
        for speaker_id in speaker_ids:
            if speaker_id == -1:
                ...
                if re.match("^SparseLibri(2|3|23)Mix$|^Libri(2|3|23)Mix$", self.dataset_name):
                    path = os.path.join(self.spk_path, self.filename2aux[file][speaker_id] + '.pt')
                elif self.dataset_name == 'icmc':
                    if self.is_train:
                        path = os.path.join(self.spk_path, self.data2spk[f"{file}/{speaker_id}"] + '.pt')
                    else:
                        path = os.path.join(self.spk_path, file, self.data2spk[f"{file}/{speaker_id}"] + '.pt')
                else:
    ...
    def load_ts(self, file, speaker_ids, rc=0):
        ...
        for speaker_id in speaker_ids:
            if speaker_id != -1 and speaker_id != -2:
                if self.dataset_name.startswith('Libri') and self.dataset_name.endswith('Mix') or self.dataset_name.startswith('SparseLibri'):
                    exist_spk.append(self.data2spk[self.filename2aux[file][speaker_id]])
        ...
        for speaker_id in speaker_ids:
            if speaker_id == -1:
                if np.random.choice(2, p=[1 - self.zero_ratio, self.zero_ratio]) == 0 and self.is_train:
                    random_spk = random.choice(list(self.spk2data))
                    while random_spk in exist_spk:
                        random_spk = random.choice(list(self.spk2data))
                    exist_spk.append(random_spk)
                    ...
                    if self.dataset_name.startswith('Libri') and self.dataset_name.endswith('Mix') or self.dataset_name.startswith('SparseLibri'):
                        if len(random_speech.split('-')) > 1:
                            ...
                            path = os.path.join(self.aux_path, f"{spk}/{chapter}/{random_speech}.flac")
                        else:
                            spk = random_speech
                ...
                if self.dataset_name.startswith('Libri') and self.dataset_name.endswith('Mix') or self.dataset_name.startswith('SparseLibri'):
                    if len(self.filename2aux[file][speaker_id].split('-')) > 1:
                        ...
                        path = os.path.join(self.aux_path, f"{spk}/{chapter}/{self.filename2aux[file][speaker_id]}.flac")
                    else:
                        spk = self.filename2aux[file][speaker_id]
                        path = os.path.join(self.aux_path, f"{self.filename2aux[file][speaker_id]}.wav")
                elif self.dataset_name == 'libri_css_sim':
                    spk = self.id2fullid[file][str(speaker_id)]
                    path = os.path.join(self.aux_path, file, f"{self.id2fullid[file][str(speaker_id)]}.wav")
            ...
            if path is not None and librosa.get_duration(path=path) > 0.01:
                ...
                if aux_len <= self.ts_len:
                    ...
                    if self.is_train:
                        start_frame = np.int64(random.random()*(aux_len-self.ts_len) * sr_cur)
                    else:
                        start_frame = 0
                    if self.inference:
                        target_speech, _ = self.read_audio_with_resample(path, sr_cur=sr_cur, rc=rc)
                    else:
                        target_speech, _ = self.read_audio_with_resample(path, start=start_frame, length=int(self.ts_len * sr_cur), sr_cur=sr_cur, rc=rc)
        ...
        target_len = torch.tensor([ts.size(0) for ts in target_speeches], dtype=torch.long)
    ...
    def collater(self, samples):
        ...
        labels_len = torch.tensor([s["labels"].size(1) for s in samples], dtype=torch.long)
    ...
    def add_noise(self, audio, noisecat, length):
        ...
        for noise in noiselist:
            ...
            if noiselength <= length:
                ...
                noiseaudio, _  = self.read_audio_with_resample(noise, start=start_frame, length=length)
    ...
    def read_audio_with_resample(self, audio_path, start=None, length=None, sr_cur=None, support_mc=False, rc=-1):
        if sr_cur is None:
            sr_cur = self.sample_rate
        ...
        if audio_sr != self.sample_rate:
            try:
                if start is not None:
                    audio, _ = librosa.load(audio_path, offset=start / sr_cur, duration=length / sr_cur, sr=sr_cur, mono=False)
                else:
            ...
            try:
                if start is not None:
                    audio, _ = sf.read(audio_path, start=start, stop=start + length, dtype='float32')
                else:
