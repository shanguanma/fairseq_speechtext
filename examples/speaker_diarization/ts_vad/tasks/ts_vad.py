...
class TSVADTaskConfig(FairseqDataclass):
    ...
    aux_path: Optional[str] = field(
        default=None,
        metadata={"help": "path to aux audio directory."}
    )
    ...
    collar: float = field(
        default=0.25,
        metadata={"help": "collar value"}
    )
    med_filter: int = field(
        default=21,
        metadata={"help": "med filter"}
    )
    embed_input: bool = field(
        default=False,
        metadata={"help": "embedding input"}
    )
    embed_len: float = field(
        default=1,
        metadata={"help": "embedding length for diarization"}
    )
    embed_shift: float = field(
        default=0.4,
        metadata={"help": "embedding shift for diarization"}
    )
    librimix_mode: str = field(
        default='min',
        metadata={"help": "librimix mode"}
    )
    librimix_type: str = field(
        default='both',
        metadata={"help": "both or clean"}
    )
    mix_noise: bool = field(
        default=False,
        metadata={"help": "add noise to mix"}
    )
    aux_noise: bool = field(
        default=False,
        metadata={"help": "add noise to aux"}
    )
    ovr_ratio: Optional[float] = field(
        default=None,
        metadata={"help": "overlap ratio for the SparseLibriMix dataset"}
    )
    label_rate: int = field(
        default=25,
        metadata={"help": "diarization label rate"}
    )
    residual_predict: bool = field(
        default=False,
        metadata={"help": "whether to predict all left audio"}
    )
    random_channel: bool = field(
        default=False,
        metadata={"help": "for multi-channel, use a randomly one"}
    )
    support_mc: bool = field(
        default=False,
        metadata={"help": "whether use multi-channel data for icmc"}
    )
    random_mask_speaker_prob: float = field(
        default=0.0,
        metadata={"help": "whether random mask speaker from input"}
    )
    ...
    rttm_name: Optional[str] = field(
        default=None,
        metadata={"help": "rttm name"}
    )
    fusion_inf: bool = field(
        default=False,
        metadata={"help": "fusion diar for extraction"}
    )
...
class TSVADTask(FairseqTask):
    ...
    def load_dataset(
        ...
        **kwargs,
        ...
        # For librimix name
        if self.cfg.dataset_name.startswith('libri') and self.cfg.dataset_name.endswith('mix'):
            self.cfg.dataset_name = self.cfg.dataset_name.capitalize()
            self.cfg.dataset_name = self.cfg.dataset_name[:-3] + self.cfg.dataset_name[-3:].capitalize()
        ...
        if self.cfg.dataset_name == "alimeeting":
        ...
        elif self.cfg.dataset_name.startswith('Libri') and self.cfg.dataset_name.endswith('Mix'):
            ...
            assert self.cfg.ovr_ratio is not None, f"For SparseLibri(2|3|23)Mix dataset, ovr_ratio must be provided."
            if self.cfg.ovr_ratio.is_integer(): # For some reason ovr_ratio is 1.0 but it should be 1
            ...
            raise Exception(f"The given dataset {self.cfg.dataset_name} is not supported.")
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, *args, **kwargs):
