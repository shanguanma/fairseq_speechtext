...
def get_dataset_itr(args, task):
    return task.get_batch_iterator_for_eval(
...
def main(args, task=None, model_state=None):
    ...
    logger.info(f"dataset length: {len(itr)}")
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        wps_meter = TimeMeter()
        #logger.info(f"t: {t}, its len: {len(t)}")
        for sample in t:
    ...
    if args.dump_emissions:
        ...
        logging.info(f"saved {len(emissions)} emissions to {args.dump_emissions}")
    elif args.dump_features:
        feat_arr = []
        for i in range(len(features)):
            feat_arr.append(features[i])
        np.save(args.dump_features, feat_arr)
        logging.info(f"saved {len(features)} emissions to {args.dump_features}")
    else:
        if lengths_t > 0:
        ...
        logging.info("| Generate {} with beam={}".format(args.gen_subset, args.beam))
    return task, wer
