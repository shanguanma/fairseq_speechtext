...
@dataclass
class StHubertAsrConfig(FairseqDataclass):
...
@register_model("sthubert_ctc", dataclass=StHubertCtcConfig)
class HubertCtc(BaseFairseqModel):
    def __init__(self, cfg: StHubertCtcConfig, w2v_encoder: BaseFairseqModel):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: StHubertCtcConfig, task: FairseqTask):
        """Build a new model instance."""
        w2v_encoder = StHubertEncoder(cfg, task)
...
class StHubertEncoder(FairseqEncoder):
    def __init__(self, cfg: StHubertAsrConfig, task):
