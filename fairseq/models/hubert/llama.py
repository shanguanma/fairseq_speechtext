...
"""
reference: https://arxiv.org/pdf/2310.12973.pdf
remove the rotary positional embedding and auto-regressive attention masks
modified from https://github.com/ziqipang/LM4VisualEncoding/blob/main/image_classification/models/llama.py
"""
    ...
    def custom_load_state_dict(self, checkpoint, tail=False, strict=False):
        ...
        if tail:
            for i in range(self.first_layer,self.n_layers): ##(TODO) md modify more correct name i.e. select layer? 
                ...
                layer_checkpoint = {k: checkpoint[f'layers.{i}.{k}'] for k in layer_checkpoint_keys} ## weight 
                self.layers[i - self.first_layer].load_state_dict(
                    layer_checkpoint, strict=strict)             
        return
