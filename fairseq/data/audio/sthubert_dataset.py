...
def load_text_offset(text_path, inds, tot):
    with open(text_path, "r") as f:
        ...
        assert (
            len(text_code_lengths) == tot
        ), f"number of text utterance does not match ({len(text_code_lengths)} != {tot})"
...
class StHubertDataset(FairseqDataset):
    def __init__(
        self,
        manifest_path: str,
        sample_rate: float,
        ...
        if store_labels:
            ...
            self.text_offsets_list = [
                load_text_offset(p, inds, tot) for p in text_paths
            ]
        logger.info(
            f"label_processors :{label_processors}, len(label_processors) : {len(label_processors)}"
        )
    ...
    def get_text(self, index, text_idx):
        ...
        if self.label_processors is not None:
            text_code = self.label_processors[text_idx](
                text_code
            )  ## its function is encode utterance into 1-dimtensor
        return text_code
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "label_list": labels,
        }  ## sample contents ##(TODO): check if text need to pad
    ...
    def collater_frm_label(self, targets, audio_size, audio_starts, label_rate, pad):
        ...
        # logger.info(f"in collater_frm_label , labels: {targets}")
    ...
    def collater_text(self, sources_by_text, audio_size, audio_starts):
        ...
        for texts, label_rate, pad in itr:
            # logger.info(f"in collater_text,texts: {texts}")
            if label_rate != -1.0:
